  ...
  <div class="relative w-full h-full min-h-[85vh] flex flex-col items-center justify-center font-Montserrat text-white">
    ...
    <div
      ...
      <form @submit.prevent="handleSubmit" class="flex flex-col w-full gap-4">
        ...
        <div class="w-full">
          <input
            class="w-full bg-blue-3/50 border border-blue-4/30 rounded-xl p-3 placeholder-grey-1 focus:ring-2 focus:ring-purple focus:border-purple focus:outline-none text-white transition-all"
        ...
        <div class="w-full">
          <input
            class="w-full bg-blue-3/50 border border-blue-4/30 rounded-xl p-3 placeholder-grey-1 focus:ring-2 focus:ring-purple focus:border-purple focus:outline-none text-white transition-all"
            v-model="password" placeholder="Password" type="password">
          <div v-show="submitted && !password" class="text-yellow text-xs mt-1 ml-1">Password is required</div>
        </div>

        <div v-if="error"
          class="text-[#f87171] text-sm text-center bg-blue-3/30 p-2 rounded-lg border border-[#f87171]/30">{{ error }}
        </div>

        <button
          class="w-full bg-purple hover:bg-blue-4 text-blue-1 py-3 rounded-xl font-bold text-lg transition-all duration-300 hover:shadow-[0_0_15px_rgba(153,163,251,0.4)] mt-4 active:scale-95"
          type="submit">
          Log In
        </button>
